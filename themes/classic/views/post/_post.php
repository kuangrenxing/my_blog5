...
<?php if(!$model->titleLink):?>
    <div class="title">
        <h2><?php echo CHtml::link(CHtml::encode($model->title),array('post/show','slug'=>$model->slug)); ?></h2>
    </div>
<?php else:?>
    <div class="titlelink">
        <h2><?php echo CHtml::link(CHtml::encode($model->title),$model->titleLink); ?><h2>
    </div>
<?php endif;?>

<div class="author">
    <?php echo Yii::t('lan','posted by'); ?>
    <?php echo (isset($model->author->username)?$model->author->username:$model->authorName).'
        '.Yii::t('lan','on').' '.Yii::t('lan',date('F',$model->createTime)).date(' j, Y',$model->createTime);?>
</div>
...
<?php echo CHtml::linkButton(Yii::t('lan','Delete'),array(
        ...
        'confirm'=>Yii::t('lan','Are you sure to detete this post?'),
));?>
<?php endif;?>
