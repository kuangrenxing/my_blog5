<div class="post">
    <div class="title">
     ...
     <div class="author">
        ...
        <?php echo (($model->author->username) ? CHtml::link($model->author->username,array('user/show', 'id'=>$model->authorId)):$model->authorName).' '.Yii::t('lan','on').' '.Yii::t('lan',date('F',$model->createTime)).date(' j, Y',$model->createTime); ?>
    </div>
        <f> <?php echo $readFlag==0? '切换:':'Switch&nbsp;Language&nbsp;to:'?> </f>  
    <a href="<?php echo Yii::app()->baseUrl; ?>/post/show/slug/<?php echo $model->slug;?>?readFlag=<?php if($readFlag==0){echo '1';}else{echo '0';} ?>">
